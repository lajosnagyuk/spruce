...
service="http://${fullname}.${namespace}.svc:8080"
run_case() {
  name=$1; shift
...
spec:
  ...
  template:
    metadata:
      labels:
        app.kubernetes.io/name: spruce-test
      ...
      containers:
        - name: integration
          ...
          env:
            - name: SPRUCE_TOKEN
              valueFrom:
                secretKeyRef: {name: $auth, key: client-token}" | kubectl --context "$context" apply -f - >/dev/null
...
traffic=$!
sleep 2
kubectl --context "$context" -n "$namespace" rollout restart statefulset/$fullname >/dev/null
kubectl --context "$context" -n "$namespace" rollout status statefulset/$fullname --timeout=180s >/dev/null
kubectl --context "$context" -n "$namespace" rollout restart deployment/${fullname}-gateway >/dev/null
kubectl --context "$context" -n "$namespace" rollout status deployment/${fullname}-gateway --timeout=180s >/dev/null
wait "$traffic"
...
traffic=$!
sleep 2
kubectl --context "$context" -n "$namespace" delete pod ${fullname}-1 --wait=false >/dev/null
kubectl --context "$context" -n "$namespace" rollout status statefulset/$fullname --timeout=180s >/dev/null
wait "$traffic"
