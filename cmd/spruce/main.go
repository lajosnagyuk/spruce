...
func main() {
	...
	cfg.StreamMemoryBytes = envInt64("SPRUCE_STREAM_MEMORY_BYTES", cfg.StreamMemoryBytes)
	...
	if cfg.CacheBytes <= 0 || cfg.MaxMessage <= 0 || cfg.QueueDepth <= 0 || cfg.DefaultTTL <= 0 || cfg.MaxTTL <= 0 || cfg.AckDeadline <= 0 || cfg.ReplicationQueueBytes <= 0 || cfg.ActionQueueBytes <= 0 || cfg.MaxInflightBytes <= 0 || cfg.MaxSubscriberInflightBytes <= 0 || cfg.MaxAttempts <= 0 || cfg.IdempotencyEntries <= 0 || cfg.CheckpointEntries <= 0 || cfg.MaxConcurrentRequests <= 0 || cfg.MaxStreams <= 0 || cfg.StreamMemoryBytes < 256<<10 || cfg.MaxInternalRequests <= 0 || cfg.PublishAdmissionBytes < cfg.MaxMessage || cfg.PublishAdmissionBytes < 16<<20 || cfg.PublishAdmissionWait <= 0 || cfg.DeliveryLagLimit <= 0 || cfg.DefaultTTL > cfg.MaxTTL {
